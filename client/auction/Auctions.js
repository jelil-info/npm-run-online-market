...
const useStyles = makeStyles(theme => ({
  ...
  auctionDetails:{ marginRight:'200px' },
  auctionEdit: {backgroundColor:'red', },
  ...
  const auctionState = (auction)=>{
    ...
    <NoSSR>
        ...
        {props.auctions.map((auction, i) => {
            return   <span key={i}>
              <ListItem className={classes.auctionFlex} button>
                <ListItemAvatar className={classes.auctionPicture}>
                  <Avatar variant='square' src={'/api/auctions/image/'+auction._id+"?" + new Date().getTime()}/>
                </ListItemAvatar>
                <ListItemText className={classes.auctionDetails} primary={auction.itemName} secondary={auctionState(auction)}/>
                <ListItemSecondaryAction className={classes.auctionEdit}>
